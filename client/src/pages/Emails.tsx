...
export default function Emails() {
  ...
  const { data: allItems, loading, error } = useAPI<EmailItem>("/emails", { limit: 200 });

  const emailsData = allItems.filter(
    (item) =>
      !searchTerm ||
  ...
  const getAcaoColor = (acao: string) => {
    ...
    if (lower.includes("proposta")) return "#FFD700";
    if (lower.includes("follow")) return "#00FF00";
    if (lower.includes("cobran")) return "#8A2BE2";
    if (lower.includes("boas-vindas") || lower.includes("welcome")) return "#00FF00";
    return "#4A90E2";
  };
        ...
        <div className="flex-1 overflow-auto p-8">
          ...
          {!loading && !error && (
            <>
                    ...
                    {emailsData.length === 0 ? (
                      ...
                      emailsData.map((item, idx) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 text-gray-300">{item.destinatario ?? item["destinatário"]}</td>
                          <td className="px-6 py-4">
              ...
              <div className="text-center text-gray-400 text-sm mt-4">{emailsData.length} registros</div>
            </>
          )}
        </div>
      </div>
