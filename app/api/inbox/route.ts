...
export async function GET(request: NextRequest) {
  ...
  const allowed = await can(session, "inbox", "view");
  if (!allowed) {
  ...
  try {
    ...
    const rows = await fetchTableRows("chegada_suporte", { limit: Math.min(limit * 4, 200) });

    // Filter by empresa access (admins see all)
